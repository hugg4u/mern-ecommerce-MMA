...
export default function StaffProPic({ staff }) {
    ...
    const handleApi = async (e) => {
        ...
        reader.onloadend = async () => {
            ...
            Toaster.loadingToast('Đang tải ảnh lên...')
            ...
            try {
                ...
                if (result.data.data) {
                    ...
                    const cloudinaryUrl = result.data.data.url;
                    setImage(cloudinaryUrl);
                    // Lưu URL vào MongoDB
                    await uploadImageToMongo(cloudinaryUrl);
                }
            } catch (error) {
    ...
    const uploadImageToMongo = async (imgUrl) => {
        ...
        try {
            ...
            if (mongoResult.data.code === 200) {
                Toaster.justToast('success', "Ảnh đã được tải lên thành công", () => {
                    setImage(imgUrl)
                })
            }
        } catch (error) {
